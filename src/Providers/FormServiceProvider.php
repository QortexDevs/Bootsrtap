    ...
    public function boot()
    {
        ...
        Form::macro('cpFilterSelectMultiple', function ($label, $name, $values = [], $value = null, $attributes = []) use ($self) {
            if (isset($attributes['groupby'])) {
                $groups = [];
                $filterProperty = $attributes['groupby'];
                foreach ($values as $initalValue) {
                    $filterValue = $initalValue->$filterProperty;
                    if (!isset($groups[$filterValue])) {
                        $groups[$filterValue] = [];
                    }
                    $groups[$filterValue][$initalValue->id] = $initalValue->name;
                }
                if (count($groups) == 1) {
                    $values = reset($groups);
                } else {
        ...
        Form::component('checkboxFormControl', 'qortex::components.control-panel.form.checkbox', [
            'name',
            'label',
            'checked' => false,
            'required' => false,
        ...
        Form::macro('cpFormCheckbox', function ($name, $label, $checked = false, $required = false, $attributes = []) use ($self) {
            $defaultAttributes = [
